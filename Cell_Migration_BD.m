...
filename = strcat('BD_',date);
rng(datenum(date));%......................................For repeatability
tic

%Model Definitions
runTime = 48*3600;%...........................Total runtime for model (sec)
dt = 2;%.......................................Size of each time step (sec)
elmtSize = 0.01;%............Size of binding site element (um) - max is 0.3
samples = 40;%..................................Sample size to get averages
dataPnts = 20;%.............................Number of parameter data points
...
%Parametric Sweep
%=========================================================================%
parfor par=1:dataPnts
    tic

    p = [3.7,0.001,1/16,0,5.7]; 
...
%=========================================================================%
    for samp=1:samples
        ...
        %=================================================================%
        cellPos(pos,:) = 500*rand(1,3)-250;%...................Initial position
        p1(par,:,samp) = cellPos(pos,:);
        ...
        for i=dt:dt:runTime
            ...
            x = abs(cellPos(pos,1)+pseudoVect(1));
            
            if (x>0 && x<=100) || (x>200 && x<=400) || (x>500 && x<=700)
                RGD = 5;
            elseif (x>100 && x<=200) || (x>400 && x<=500) || (x>700 && x<=800)
                RGD = 5.7;
            else
                RGD = 5;
            end
            ...
            if newAngle
                ...
                s1 = (pi^(1/3)*(6*Vcell)^(2/3))/A_cell;%.Cell sphericity
                dR = dRecp/A_cell;%...........Membrane integrin density
                polAngle = 180*s1^2;......................Polarity angle
...
%=========================================================================%        
        ...
        try%.........Curve fit to get random motility coefficient and alpha
            ...
            c = coeffvalues(f);
            D(par,samp) = c(1);%................Random motility coefficient 
            alp(par,samp) = c(2);%....................................Alpha
            R2(par,samp) = g.rsquare;%......................Goodness of fit
        catch
...
save(filename,'avgVel','cellDist','LpR','D','alp','R1','R2','R3','outg','cont','retr','p1','p2')
%delete(gcp('nocreate'))
...
for i=1:size(p1,1)
    for j=1:size(p1,3)
        ...
        x = abs(p2(i,1,j));
        scatter3(p1(i,1,j),p1(i,2,j),p1(i,3,j),'g','filled')
        if (x>0 && x<=100) || (x>200 && x<=400) || (x>500 && x<=700)
            scatter3(p2(i,1,j),p2(i,2,j),p2(i,3,j),50,'b','filled')
            n2=n2+1;
        elseif (x>100 && x<=200) || (x>400 && x<=500) || (x>700 && x<=800)
            scatter3(p2(i,1,j),p2(i,2,j),p2(i,3,j),'r','filled')
            n1=n1+1;
        else
            scatter3(p2(i,1,j),p2(i,2,j),p2(i,3,j),'b','filled')
            n2=n2+1;
        end
        line([p1(i,1,j),p2(i,1,j)],[p1(i,2,j),p2(i,2,j)],[p1(i,3,j),p2(i,3,j)])
